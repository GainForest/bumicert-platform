...
import { truncateEthereumAddress } from "@/lib/truncateEthereumAddress";
import { Check, Copy } from "lucide-react";
import Link from "next/link";
import React, { useCallback, useLayoutEffect, useRef, useState } from "react";
import { useEnsName } from "wagmi";
import EthAvatar from "./eth-avatar";
...
const UserChip = ({
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      const computed = computeMiddleEllipsis(did);
  ...
  }, [did, ellipsisLocation, computeMiddleEllipsis]);

  const displayValue = ellipsisLocation === "middle" ? middleEllipsis : did;
  ...
  return (
    ...
    >
      ...
      >
        {showAvatar && <UserAvatar did={did} size={avatarSize} />}
        <Link
          href={links.user(did)}
        ...
        >
          <input
            ...
            className={cn(
              ...
              "group-focus/user-chip-link:underline group-hover/user-chip-link:underline"
            )}
            value={displayValue}
