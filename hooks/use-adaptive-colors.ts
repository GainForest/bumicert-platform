...
// Enhanced useAdaptiveColors with two-color fallback
export function useAdaptiveColors(imageSrc: string | null): AdaptiveColors {
  ...
  const { data } = useQuery({
    ...
    queryFn: async (): Promise<ExtractedColors | null> => {
      ...
      if (colors.length >= 2) {
        const [bg] = colors.sort((a, b) => b.area - a.area); // Dominant first
        const lightness = bg.lightness;

        // Fallback logic for poor contrast scenarios
        if (colors.every((c) => Math.abs(c.lightness - 0.5) < 0.3)) {
          ...
          // Normal case: pick contrasting pair
...
export const getContrast = (hex1: HexColor, hex2: HexColor): number => {
  ...
  const lum1 = rgbToRelativeLuminance(rgb1);
  const lum2 = rgbToRelativeLuminance(rgb2);

  return (Math.max(lum1, lum2) + 0.05) / (Math.min(lum1, lum2) + 0.05);
};

// For getting other shades:
...
const adjustLightness = (hex: HexColor, delta: number): HexColor => {
  ...
  hsl[2] = Math.max(0, Math.min(1, hsl[2] + delta)); // Lightness 0-1
  return hslToHex(hsl);
};
