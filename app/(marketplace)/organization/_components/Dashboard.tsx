...
import { OrgHypercertsDefs } from "climateai-sdk/lex-api";
                ...
                <div className="w-full bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        {(() => {
                            ...
                            if (logo && logo.$type === "app.gainforest.common.defs#smallImage") {
                                // Cast to specific type safely if needed, but in TS regular check is often enough if types are discriminated
                                // However, based on error logs, `logo` structure needs careful handling.
                                // Assuming logo.image is the blob.
                                const smallImage = logo as OrgHypercertsDefs.SmallImage;
                                if (smallImage.image) {
                                    logoUrl = getBlobUrl(did, smallImage.image, allowedPDSDomains[0]);
                                }
                            }
                            ...
                            if (logoUrl) {
                                return (
                                    <img
                                        src={logoUrl}
                                        alt={organizationInfo.displayName || "Organization Logo"}
