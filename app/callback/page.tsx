...
const CallbackPage = () => {
  ...
  const { isReady, isAuthenticated, authError, refreshSession } = useAtproto();
    ...
    if (typeof window !== "undefined") {
      ...
      if (state) {
        if (state.length < 10 || !/^[a-zA-Z0-9_-]+$/.test(state)) {
          setCsrfError(
            "Invalid state parameter format - possible security issue"
          );
          return;
        }
      }
      ...
      if (suspiciousParams.length > 0) {
        setCsrfError(
          "Suspicious parameters detected - possible security attack"
        );
        return;
      }
    }
